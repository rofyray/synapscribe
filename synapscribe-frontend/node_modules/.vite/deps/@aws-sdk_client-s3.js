...
function locationConstraintMiddleware(options) {
  return (next) => async (args) => {
    const { CreateBucketConfiguration: CreateBucketConfiguration2 } = args.input;
    const region = await options.region();
    if (!CreateBucketConfiguration2?.LocationConstraint && !CreateBucketConfiguration2?.Location) {
      if (region !== "us-east-1") {
